...
function getBuddyListFriends(list: BuddyList): Friend[] { //returns friend a 2nd time if friend is also a colleague
    const friends: Friend[] = [];
    for (const b of list.members) {
         if (b.hasOwnProperty("phone")) {
              friends.push(b as Friend);
         }
     }
    return friends;
